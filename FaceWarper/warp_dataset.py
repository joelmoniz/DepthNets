...
class Dataset:
	...
	def depth_filepath(self, index):
		return os.path.join(self._dataset_dir, "depth", "depth_" + str(index) + ".txt")

	def affine_filepath(self, index):
		return os.path.join(self._dataset_dir, "affine", "aff_" + str(index) + ".txt")
...
class ResultsDestination:
	...
	def result_filepath(self, index):
		return os.path.join(self._path, "affine_" + str(index) + ".png")
...
def warp_all(server, dataset, results_destination, options):
	...
	with open(dataset.pairs_filepath()) as f:
		for i, line in enumerate(f):
			...
			depth = dataset.depth_filepath(file_index)
			if options.identity :
				affine = dataset.affine_identity_filepath()
			else:
				affine = dataset.affine_filepath(file_index)
			result = results_destination.result_filepath(file_index)
