...
def image_filepath(fileID):
    return os.path.join(DATASET_DIR, "images", fileID + ".png")

def keypoints_filepath(fileID):
    return os.path.join(DATASET_DIR, "keypoints", fileID + ".txt")

def depth_filepath(fileID):
    return os.path.join(DATASET_DIR, "depth_anaff_frontal", "depth", fileID + ".txt")

def affine_filepath(fileID):
    return os.path.join(DATASET_DIR, "depth_anaff_frontal", "affine", fileID + ".txt")  
...
def main():
    ...
    for person in os.listdir(os.path.join(DATASET_DIR,"images/")):
        #picture_path = DATASET_DIR + "images/" + person
        picture_path = os.path.join(DATASET_DIR, "images", person)
        ...
        if options.identity:
            ...
            depth = depth_filepath(fileID)
            affine = affine_filepath(fileID)
       
        result = result_filepath(fileID, results_dir)
